...
>>>>>>> pravallika
  ...
  data.opsReadiness = opsRecord || opsRecords || null;
  data.opsRecord = data.opsReadiness;
...
function isOpsReadinessApproved(data) {
  return !!(data.opsReadiness && (data.opsReadiness.validation || data.opsReadiness.handover || data.opsReadiness.monitoring));
}

function determineCurrentStage(data) {
  if (isOpsReadinessApproved(data)) return 'ops-readiness';
...
function calculateProgress(dataOrStage) {
  if (typeof dataOrStage === 'string') {
    const stages = [
    ...
    const idx = stages.indexOf(dataOrStage);
    return Math.round(((idx + 1) / stages.length) * 100);
  }
  const data = dataOrStage || {};
  let completed = 0;
  ...
  if (isOpsReadinessApproved(data)) completed++;
...
const renderOpsReadinessCard = renderOpsCard;
