...
window.fetchOpsReadinessData = async function () {
  ...
  try {
    ...
    if (opsDemands.length > 0) {
      if (!opsSelectedDemandId || !opsDemands.some(d => d.demand_id === opsSelectedDemandId)) {
...
function renderMonitoringSetup() {
  ...
  let formHtml = `
    <div class="ops-card">
      <div class="ops-card-title">Configure Monitoring Policy & Scope</div>
      <div class="ops-grid">
        <div class="ops-form-group">
          <label for="mon-components">Components to Monitor (CMDB & SDLC Scope)</label>
          <input type="text" id="mon-components" value="${defaultComponents}">
        </div>
        <div class="ops-form-group">
      ...
      <div class="ops-grid">
        <div class="ops-form-group">
          <label for="mon-availability">Target Availability SLO (%)</label>
          <input type="number" id="mon-availability" step="0.01" value="99.95">
        </div>
        <div class="ops-form-group">
          <label for="mon-latency">Target p99 Latency SLO (ms)</label>
          <input type="number" id="mon-latency" value="500">
        </div>
      </div>
  ...
  if (mon) {
    ...
    const alertsList = (mon.proposed_alerts || []).map(a => {
      ...
      if (cLower.includes('db') || cLower.includes('mongo') || cLower.includes('database')) icon = '💾';
      ...
      return `
        <div class="ops-check-item" style="flex-direction: column; align-items: flex-start; gap: 0.25rem; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 4px; background: rgba(0,0,0,0.15); margin-bottom: 0.5rem;">
          <div style="display: flex; width:100%; justify-content: space-between; align-items: center;">
            ...
            <span class="ops-pill fail" style="font-size:0.6rem; padding:1px 5px;">${a.severity}</span>
          </div>
          ...
          <div style="font-size:0.72rem; color: var(--text-muted); margin-top:0.15rem;">
            <strong>Notifications:</strong> ${(a.notify || []).join(', ')}
          </div>
        </div>
      `;
    }).join('');
      ...
      <div class="ops-check-item" style="flex-direction: column; align-items: flex-start; gap: 0.5rem; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 4px; background: rgba(0,0,0,0.15);">
        ...
        ${d.widgets && d.widgets.length > 0 ? `
          <div style="margin-top: 0.4rem; width: 100%; border-top: 1px dashed var(--border-color); padding-top: 0.4rem;">
            <div style="font-size:0.72rem; color:var(--text-muted); margin-bottom: 0.3rem;">Widget Specifications:</div>
            ${d.widgets.map(w => `
    ...
    resultsHtml = `
      <div class="ops-card">
        <div class="ops-card-title" style="display:flex; justify-content:space-between; align-items:center;">
          <span>Persisted Monitoring Plan Artifact (MON-${suffix})</span>
        ...
        <div style="margin-bottom: 1rem; padding: 0.6rem 0.8rem; background: rgba(0,0,0,0.2); border-radius: 4px; border: 1px solid var(--border-color);">
          <div style="font-size:0.78rem; color:var(--text-secondary); margin-bottom: 0.4rem;">
            <strong>Full Monitored Components Scope:</strong>
          </div>
          <div style="display:flex; flex-wrap:wrap; gap:0.3rem;">
        ...
        <div class="ops-grid">
          <div>
            <h4 style="margin:0 0 0.5rem 0; font-size: 0.85rem; color: var(--text-primary); border-bottom:1px solid var(--border-color); padding-bottom:0.3rem;">Component-Specific Alert Rules</h4>
            ${alertsList || '<p style="color:var(--text-muted); font-size:0.8rem;">No alerts proposed</p>'}
          </div>
          <div>
            <h4 style="margin:0 0 0.5rem 0; font-size: 0.85rem; color: var(--text-primary); border-bottom:1px solid var(--border-color); padding-bottom:0.3rem;">Widget-Based Dashboard Specifications</h4>
        ...
        ${!mon.sre_reviewed ? `
          <div style="border-top:1px solid var(--border-color); padding-top: 1rem; margin-top:1.5rem; display: flex; justify-content: flex-end; gap: 0.5rem; align-items: center;">
            ...
            <button class="ops-btn" id="ops-sre-approve-btn">Approve Monitoring Plan (MON-${suffix})</button>
          </div>
        ` : `
          <div style="border-top:1px solid var(--border-color); padding-top: 1rem; margin-top:1.5rem; font-size: 0.8rem; color: var(--color-status-green-text); font-weight: bold; text-align: right;">
            ✓ Monitoring Plan MON-${suffix} signed off by ${mon.sre_reviewed_by || 'SRE Lead'}. Ready for Stage 09-A Validation.
          </div>
        `}
      </div>
    `;
...
async function generateMonitoringConfig() {
  ...
  const availability = parseFloat(document.getElementById('mon-availability').value) || 99.95;
  const latency = parseInt(document.getElementById('mon-latency').value) || 500;

  const component_ids = compStr.split(',').map(c => c.trim()).filter(Boolean);

  const slos = component_ids.map(c => ({
    component_id: c,
