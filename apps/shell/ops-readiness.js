...
function renderMonitoringSetup() {
  ...
  if (derivedComponents.length === 0) {
    derivedComponents = ['svc-ecom-chatbot', 'mongodb', 'kafka', 'oauth-service'];
  }
      ...
      <div>
        ...
        ${mon ? `
          <div style="font-size:0.75rem; color:var(--text-muted); margin-top:0.2rem;">
            Monitoring Plan: <strong>${mon.monitoring_plan_id || 'MON-PLAN-' + suffix}</strong> | Release: <strong>${mon.release_id || 'REL-' + suffix + '-1'}</strong>
          </div>
        ` : ''}
      </div>
      <div>
  ...
  let formHtml = `
    <div class="ops-card">
      <div class="ops-card-title">Configure Scope & Trigger AI Agent</div>
      <div class="ops-grid">
        <div class="ops-form-group">
          <label for="mon-components">Components to Monitor (SDLC Scope)</label>
      ...
      <button class="ops-btn" id="ops-gen-mon-btn">Generate AI Monitoring Plan</button>
    </div>
  `;

  let resultsHtml = '';
  if (mon) {
    const scopeComponents = mon.monitored_components_scope || mon.component_ids || [];
    const scopePills = scopeComponents.map(c => `
      <span class="ops-pill info" style="font-size:0.7rem; padding:3px 8px; background: rgba(99, 102, 241, 0.12); color: #818cf8; border: 1px solid rgba(99, 102, 241, 0.3); text-transform:none;"><code>${c}</code></span>
    `).join(' ');

    // Determine detected technology categories for Section 3 Monitoring Coverage
    const specs = mon.component_specs || [];
    const compTypes = new Set(specs.map(s => (s.component_type || '').toLowerCase()));
    
    let hasApp = false, hasDb = false, hasMsg = false, hasApi = false, hasInfra = true;
    if (specs.length === 0) {
      hasApp = true; hasDb = true; hasMsg = true; hasApi = true;
    } else {
      specs.forEach(s => {
        const t = (s.component_type || '').toLowerCase();
        const cid = (s.component_id || '').toLowerCase();
        if (t === 'rest_api' || t === 'service' || cid.includes('svc') || cid.includes('api')) hasApp = true;
        if (t === 'postgresql' || t === 'mongodb' || t === 'database' || cid.includes('db') || cid.includes('mongo') || cid.includes('sql')) hasDb = true;
        if (t === 'kafka' || t === 'queue' || t === 'rabbitmq' || cid.includes('kafka') || cid.includes('queue') || cid.includes('mq')) hasMsg = true;
        if (cid.includes('auth') || cid.includes('oauth') || cid.includes('sso') || cid.includes('gateway') || cid.includes('external')) hasApi = true;
        if (t === 'infrastructure' || cid.includes('infra') || cid.includes('cluster') || cid.includes('node')) hasInfra = true;
      });
    }

    // 1. Monitoring Scope Section HTML
    const section1_Scope = `
      <div class="ops-card" style="border-left: 3px solid #6366f1;">
        <div class="ops-card-title" style="display:flex; justify-content:space-between; align-items:center;">
          <span>1. Monitoring Scope</span>
          <span style="font-size:0.75rem; color:var(--text-muted);">Deployment Scope Identified</span>
        </div>
        <div class="ops-grid" style="grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 0.75rem; text-align: center;">
          <div style="background: rgba(0,0,0,0.2); padding: 0.6rem; border-radius: 6px; border: 1px solid var(--border-color);">
            <div style="font-size: 0.7rem; color: var(--text-muted);">Environment</div>
            <strong style="font-size: 0.95rem; color: var(--text-primary); text-transform: uppercase;">${mon.environment || 'Production'}</strong>
          </div>
          <div style="background: rgba(0,0,0,0.2); padding: 0.6rem; border-radius: 6px; border: 1px solid var(--border-color);">
            <div style="font-size: 0.7rem; color: var(--text-muted);">Release ID</div>
            <strong style="font-size: 0.95rem; color: #818cf8;">${mon.release_id || 'REL-' + suffix + '-1'}</strong>
          </div>
          <div style="background: rgba(0,0,0,0.2); padding: 0.6rem; border-radius: 6px; border: 1px solid var(--border-color);">
            <div style="font-size: 0.7rem; color: var(--text-muted);">Monitoring Plan ID</div>
            <strong style="font-size: 0.95rem; color: #10b981;">${mon.monitoring_plan_id || 'MON-PLAN-' + suffix}</strong>
          </div>
          <div style="background: rgba(0,0,0,0.2); padding: 0.6rem; border-radius: 6px; border: 1px solid var(--border-color);">
            <div style="font-size: 0.7rem; color: var(--text-muted);">Component Count</div>
            <strong style="font-size: 1.1rem; color: #f59e0b;">${scopeComponents.length}</strong>
          </div>
        </div>
        <div style="padding: 0.6rem 0.8rem; background: rgba(0,0,0,0.15); border-radius: 4px; border: 1px solid var(--border-color);">
          <div style="font-size:0.75rem; color:var(--text-secondary); margin-bottom: 0.35rem;">
            <strong>Identified Components for Monitoring:</strong>
          </div>
          <div style="display:flex; flex-wrap:wrap; gap:0.4rem;">
            ${scopePills || '<span>No components identified</span>'}
          </div>
        </div>
      </div>
    `;

    // 2. AI Monitoring Strategy Section HTML
    const strategyItems = (mon.component_specs || []).map(cs => {
      let defaultReason = 'Core operational component in deployment scope.';
      const cid = (cs.component_id || '').toLowerCase();
      const ct = (cs.component_type || '').toLowerCase();
      if (ct === 'mongodb' || cid.includes('mongo')) defaultReason = 'Primary persistence layer for application data.';
      else if (ct === 'postgresql' || cid.includes('postgres') || cid.includes('sql')) defaultReason = 'Primary transactional relational database.';
      else if (ct === 'kafka' || cid.includes('kafka')) defaultReason = 'Processes asynchronous events & event streaming pipeline.';
      else if (cid.includes('auth') || cid.includes('oauth')) defaultReason = 'Authentication and security dependency.';
      else if (cid.includes('bot') || cid.includes('api') || cid.includes('svc')) defaultReason = 'Customer-facing API handling production traffic.';
      else if (ct === 'redis' || cid.includes('redis')) defaultReason = 'In-memory caching layer for session and fast state retrieval.';
      else if (ct === 'infrastructure' || cid.includes('infra')) defaultReason = 'Cluster container node host infrastructure.';

      const reasonText = cs.reason || defaultReason;
      const crit = (cs.criticality || 'standard').toLowerCase();
      const pillClass = crit === 'critical' ? 'fail' : (crit === 'high' ? 'warn' : 'pass');

      return `
        <div style="background: rgba(0,0,0,0.2); border: 1px solid var(--border-color); border-radius: 6px; padding: 0.75rem; margin-bottom: 0.5rem;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.3rem;">
            <strong style="color:var(--text-primary); font-size:0.88rem;"><code>${cs.component_id}</code></strong>
            <span class="ops-pill ${pillClass}" style="font-size:0.6rem; padding:1px 6px;">${crit.toUpperCase()}</span>
          </div>
          <div style="font-size:0.75rem; color:var(--text-muted); margin-bottom:0.35rem;">
            Tech Stack: <em style="color:var(--text-secondary);">${cs.technology_stack || 'Service'}</em> | Owner: <code>${cs.owner_team || 'Engineering'}</code>
          </div>
          <div style="font-size:0.8rem; color:#818cf8; background:rgba(99,102,241,0.08); padding:0.4rem 0.6rem; border-radius:4px; border-left:2px solid #6366f1;">
    ...
    const section2_Strategy = `
      <div class="ops-card">
        <div class="ops-card-title">2. AI Monitoring Strategy</div>
        <div style="font-size:0.78rem; color:var(--text-secondary); margin-bottom:0.75rem;">
          Architectural analysis explaining why each component requires operational monitoring:
        </div>
        ${strategyItems || '<p style="color:var(--text-muted); font-size:0.8rem;">No component strategy generated.</p>'}
      </div>
    `;

    // 3. Monitoring Coverage Section HTML
    const section3_Coverage = `
      <div class="ops-card">
        <div class="ops-card-title">3. Monitoring Coverage Summary</div>
        <div style="display:flex; flex-wrap:wrap; gap:0.75rem;">
          <div style="flex:1; min-width:140px; background:${hasApp ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)'}; border:1px solid ${hasApp ? 'rgba(16,185,129,0.3)' : 'var(--border-color)'}; padding:0.6rem; border-radius:6px; font-size:0.82rem; font-weight:600; color:${hasApp ? '#10b981' : 'var(--text-muted)'}; text-align:center;">
            ${hasApp ? '✓' : '—'} Application Services
          </div>
          <div style="flex:1; min-width:140px; background:${hasDb ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)'}; border:1px solid ${hasDb ? 'rgba(16,185,129,0.3)' : 'var(--border-color)'}; padding:0.6rem; border-radius:6px; font-size:0.82rem; font-weight:600; color:${hasDb ? '#10b981' : 'var(--text-muted)'}; text-align:center;">
            ${hasDb ? '✓' : '—'} Databases
          </div>
          <div style="flex:1; min-width:140px; background:${hasMsg ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)'}; border:1px solid ${hasMsg ? 'rgba(16,185,129,0.3)' : 'var(--border-color)'}; padding:0.6rem; border-radius:6px; font-size:0.82rem; font-weight:600; color:${hasMsg ? '#10b981' : 'var(--text-muted)'}; text-align:center;">
            ${hasMsg ? '✓' : '—'} Messaging
          </div>
          <div style="flex:1; min-width:140px; background:${hasApi ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)'}; border:1px solid ${hasApi ? 'rgba(16,185,129,0.3)' : 'var(--border-color)'}; padding:0.6rem; border-radius:6px; font-size:0.82rem; font-weight:600; color:${hasApi ? '#10b981' : 'var(--text-muted)'}; text-align:center;">
            ${hasApi ? '✓' : '—'} External APIs / Auth
          </div>
          <div style="flex:1; min-width:140px; background:${hasInfra ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.02)'}; border:1px solid ${hasInfra ? 'rgba(16,185,129,0.3)' : 'var(--border-color)'}; padding:0.6rem; border-radius:6px; font-size:0.82rem; font-weight:600; color:${hasInfra ? '#10b981' : 'var(--text-muted)'}; text-align:center;">
    ...
    // 4. AI Generated Alert Rules Section HTML
    const alertsList = (mon.proposed_alerts || []).map(a => {
      ...
      let cleanCondition = a.condition || '';
      if (cleanCondition.includes('slow_query_duration_ms') || cleanCondition.includes('slow_query')) {
        cleanCondition = 'Query execution duration exceeds configured latency baseline';
      } else if (cleanCondition.includes('p99_latency_ms') || cleanCondition.includes('latency')) {
        cleanCondition = 'Response latency exceeds configured operational threshold';
      } else if (cleanCondition.includes('availability_pct') || cleanCondition.includes('availability')) {
        cleanCondition = 'Service availability drops below required SLO threshold';
      } else if (cleanCondition.includes('error_rate_pct') || cleanCondition.includes('5xx')) {
        cleanCondition = 'HTTP 5xx server error rate exceeds configured threshold';
      } else if (cleanCondition.includes('connection_pool_utilization') || cleanCondition.includes('connection_pool')) {
        cleanCondition = 'Connection pool utilization exceeds configured operational threshold';
      } else if (cleanCondition.includes('replication_lag_seconds') || cleanCondition.includes('replication_lag')) {
        cleanCondition = 'Replica lag exceeds configured synchronization threshold';
      } else if (cleanCondition.includes('consumer_lag_messages') || cleanCondition.includes('consumer_lag')) {
        cleanCondition = 'Unconsumed message backlog exceeds configured consumer threshold';
      } else if (cleanCondition.includes('memory_utilization_pct') || cleanCondition.includes('memory')) {
        cleanCondition = 'Cache memory utilization exceeds configured capacity threshold';
      } else if (cleanCondition.includes('cache_hit_ratio_pct') || cleanCondition.includes('cache_hit')) {
        cleanCondition = 'Cache hit ratio falls below configured performance baseline';
      } else if (cleanCondition.includes('queue_depth_messages') || cleanCondition.includes('queue_depth')) {
        cleanCondition = 'Queue depth exceeds configured message backlog threshold';
      } else if (cleanCondition.includes('dlq_messages_count') || cleanCondition.includes('dead_letter')) {
        cleanCondition = 'Unprocessable messages detected in Dead Letter Queue';
      }

      let cleanName = a.name || 'Alert Rule';
      cleanName = cleanName
        .replace('Slow Query Threshold Exceeded', 'Slow Query Execution')
        .replace('p99 Latency Violation', 'High Response Latency')
        .replace('Availability Drop', 'Service Availability Drop')
        .replace('HTTP 5xx Error Rate Spike', 'High Error Rate Spike');

      const threshSrc = a.threshold_source || 'Production Monitoring Policy (MON-POLICY-v1.0)';

      return `
        <div class="ops-check-item" style="flex-direction: column; align-items: flex-start; gap: 0.25rem; padding: 0.65rem 0.75rem; border: 1px solid var(--border-color); border-radius: 4px; background: rgba(0,0,0,0.15); margin-bottom: 0.4rem;">
          <div style="display: flex; width:100%; justify-content: space-between; align-items: center;">
            <strong style="color: #818cf8; font-size:0.83rem;">${icon} ${cleanName} (<code>${a.alert_id}</code>)</strong>
            <span class="ops-pill ${a.severity === 'critical' ? 'fail' : 'warn'}" style="font-size:0.6rem; padding:1px 5px;">${a.severity}</span>
          </div>
          <div style="font-size:0.76rem; color: var(--text-primary); margin-top:0.2rem;">
            <strong>Component:</strong> <code>${a.component_id}</code>
          </div>
          <div style="font-size:0.76rem; color: var(--text-secondary); margin-top:0.1rem;">
            <strong>Trigger Condition:</strong> <code style="color:var(--color-status-amber-text);">${cleanCondition}</code>
          </div>
          <div style="display:flex; width:100%; justify-content:space-between; align-items:center; font-size:0.72rem; color: var(--text-muted); margin-top:0.25rem; border-top:1px dashed rgba(255,255,255,0.05); padding-top:0.2rem;">
            <span><strong>Notification Group:</strong> ${(a.notify || []).join(', ')}</span>
            <span style="color:#818cf8; background:rgba(99,102,241,0.08); padding:1px 6px; border-radius:3px; border:1px solid rgba(99,102,241,0.2);">Threshold Source: <code>${threshSrc}</code></span>
          </div>
        </div>
      `;
    }).join('');

    const section4_Alerts = `
      <div class="ops-card">
        <div class="ops-card-title">4. AI Generated Alert Rules</div>
        <div style="font-size:0.78rem; color:var(--text-secondary); margin-bottom:0.75rem;">
          Component-specific alert definitions generated dynamically based on component type, failure modes, and monitoring policy:
        </div>
        ${alertsList || '<p style="color:var(--text-muted); font-size:0.8rem;">No alert rules generated.</p>'}
      </div>
    `;

    // 5. Dashboard Specification Section HTML
    const dashboardWidgetsList = (mon.proposed_dashboards || []).map(d => {
      const widgetItems = (d.widgets || []).map(w => {
        let defaultWReason = 'Monitors key operational performance metric.';
        if (w.widget_id.includes('LATENCY')) defaultWReason = 'Tracks response latency percentiles to detect API degradation.';
        else if (w.widget_id.includes('ERRORS')) defaultWReason = 'Monitors HTTP 5xx error rate spikes to measure application health.';
        else if (w.widget_id.includes('CONNPOOL')) defaultWReason = 'Monitors database connection pool usage to prevent connection exhaustion.';
        else if (w.widget_id.includes('LAG')) defaultWReason = 'Tracks consumer group message lag to prevent processing backlog.';
        else if (w.widget_id.includes('RESOURCES')) defaultWReason = 'Monitors container host CPU and memory utilization against node capacity.';

        const wReason = w.reason || defaultWReason;

        return `
          <div style="font-size:0.75rem; color:var(--text-secondary); margin-bottom:0.4rem; background: rgba(0,0,0,0.2); padding:0.5rem 0.65rem; border-radius:4px; border: 1px solid var(--border-color);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.2rem;">
              <strong style="color:var(--text-primary);">${w.title}</strong>
              <span class="ops-pill info" style="font-size:0.6rem; padding:1px 5px; text-transform:uppercase;">${w.type}</span>
            </div>
            <div style="font-size:0.7rem; color:var(--text-muted); margin-bottom:0.25rem;">
              Metric: <code style="color:var(--color-brand);">${w.target_metric || 'prom_metric'}</code> | Query: <code>${w.query}</code>
            </div>
            <div style="font-size:0.73rem; color:#818cf8; background:rgba(99,102,241,0.06); padding:0.25rem 0.45rem; border-radius:3px;">
              <strong>Inclusion Rationale:</strong> ${wReason}
            </div>
          </div>
        `;
      }).join('');
      ...
      return `
        <div style="margin-bottom:0.75rem;">
          <div style="font-size:0.88rem; font-weight:600; color:var(--text-primary); margin-bottom:0.35rem;">
            📊 Dashboard Name: <span style="color:#818cf8;">${d.title}</span> (<code>${d.dashboard_id}</code>)
          </div>
          <div style="font-size:0.75rem; color:var(--text-muted); margin-bottom:0.5rem;">
            Target Technologies: <code style="color:var(--color-brand);">${d.target_technology || 'multi-tech'}</code>
          </div>
          ${widgetItems}
        </div>
      `;
    ...
    const section5_Dashboard = `
      <div class="ops-card">
        <div class="ops-card-title">5. Dashboard Specification</div>
        ${dashboardWidgetsList || '<p style="color:var(--text-muted); font-size:0.8rem;">No dashboard widgets generated.</p>'}
      </div>
    `;

    // Calculate widget count across dashboards
    let totalWidgetsCount = 0;
    (mon.proposed_dashboards || []).forEach(d => {
      totalWidgetsCount += (d.widgets || []).length;
    });

    // Determine dynamic threshold status message
    let hasDynamicBaselines = false;
    if (mon.slo_targets && mon.slo_targets.length > 0) {
      hasDynamicBaselines = mon.slo_targets.some(s => s.source === 'stage_07_load_test_baseline');
    }
    const polSummary = mon.policy_summary || {};
    const primarySrcText = polSummary.primary_threshold_source || (hasDynamicBaselines ? 'Stage 07 Performance Baseline' : 'Production Monitoring Policy (MON-POLICY-v1.0)');

    // 6. Monitoring Plan Summary Section HTML
    const section6_Summary = `
      <div class="ops-card" style="border: 1px solid rgba(99, 102, 241, 0.4); background: rgba(99, 102, 241, 0.03);">
        <div class="ops-card-title" style="display:flex; justify-content:space-between; align-items:center;">
          <span>📋 6. Monitoring Plan & Policy Summary</span>
          <span class="ops-pill ${mon.sre_reviewed ? 'pass' : 'warn'}" style="font-size:0.65rem;">
            ${mon.sre_reviewed ? 'SRE Approved' : 'Pending SRE Review'}
          </span>
        </div>
        <div class="ops-grid" style="grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 0.75rem;">
          <div style="font-size:0.82rem; color:var(--text-secondary); line-height: 1.6;">
            <div><strong>Monitoring Plan ID:</strong> <code style="color:#10b981;">${mon.monitoring_plan_id || 'MON-PLAN-' + suffix}</code></div>
            <div><strong>Release ID:</strong> <code>${mon.release_id || 'REL-' + suffix + '-1'}</code></div>
            <div><strong>Environment:</strong> <span style="text-transform:capitalize;">${mon.environment || 'Production'}</span></div>
            <div><strong>Components Covered:</strong> <strong>${scopeComponents.length}</strong></div>
            <div><strong>Alert Rules Generated:</strong> <strong>${(mon.proposed_alerts || []).length}</strong></div>
            <div><strong>Dashboard Widgets Generated:</strong> <strong>${totalWidgetsCount}</strong></div>
          </div>
          <div style="font-size:0.82rem; color:var(--text-secondary); line-height: 1.6;">
            <strong>Monitoring Coverage:</strong>
            <div style="margin-top:0.25rem; font-size:0.78rem;">
              <span style="color:${hasApp ? '#10b981' : 'var(--text-muted)'};">${hasApp ? '✓' : '—'} Application Services</span><br/>
              <span style="color:${hasDb ? '#10b981' : 'var(--text-muted)'};">${hasDb ? '✓' : '—'} Database</span><br/>
              <span style="color:${hasMsg ? '#10b981' : 'var(--text-muted)'};">${hasMsg ? '✓' : '—'} Messaging</span><br/>
              <span style="color:${hasApi ? '#10b981' : 'var(--text-muted)'};">${hasApi ? '✓' : '—'} External API</span><br/>
              <span style="color:${hasInfra ? '#10b981' : 'var(--text-muted)'};">${hasInfra ? '✓' : '—'} Infrastructure</span>
            </div>
          </div>
        </div>

        <!-- Configurable Monitoring Policy Summary Box -->
        <div style="margin-top:0.5rem; padding: 0.65rem 0.8rem; background: rgba(0,0,0,0.2); border-radius: 6px; border: 1px solid var(--border-color);">
          <h4 style="margin:0 0 0.4rem 0; font-size:0.83rem; color:var(--color-brand); border-bottom:1px solid var(--border-color); padding-bottom:0.25rem;">
            ⚙️ Monitoring Policy Summary
          </h4>
          <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:0.5rem; font-size:0.76rem; color:var(--text-secondary);">
            <div><strong>Policy Version:</strong> <code style="color:#10b981;">${polSummary.policy_version || 'MON-POLICY-v1.0'}</code></div>
            <div><strong>Policy Name:</strong> ${polSummary.policy_name || 'Production Monitoring Policy'}</div>
            <div><strong>Target Environment:</strong> ${polSummary.environment || (mon.environment || 'Production')}</div>
            <div><strong>Primary Source:</strong> <code style="color:#818cf8;">${primarySrcText}</code></div>
            <div><strong>Component Types:</strong> ${(polSummary.component_types || ['API', 'Database', 'Messaging', 'Cache', 'Infrastructure']).join(', ')}</div>
            <div><strong>Stage 07 Baseline:</strong> <span class="ops-pill ${hasDynamicBaselines ? 'pass' : 'warn'}" style="font-size:0.6rem; padding:1px 5px;">${hasDynamicBaselines ? 'Available' : 'Not Available (Policy Fallback)'}</span></div>
          </div>
        </div>
      </div>
    `;

    // 7. Approval Workflow Section HTML
    const section7_Workflow = `
      <div class="ops-card">
        <div class="ops-card-title">7. Approval Workflow</div>
        <div style="display:flex; align-items:center; justify-content:space-between; background:rgba(0,0,0,0.2); padding:0.75rem; border-radius:6px; border:1px solid var(--border-color); margin-bottom:1rem; flex-wrap:wrap; gap:0.5rem;">
          <div style="font-size:0.78rem; font-weight:600; color:#10b981; display:flex; align-items:center; gap:0.3rem;">
            <span>✓</span> Monitoring Plan Generated
          </div>
          <span style="color:var(--text-muted);">➔</span>
          <div style="font-size:0.78rem; font-weight:600; color:${mon.sre_reviewed ? '#10b981' : '#f59e0b'}; display:flex; align-items:center; gap:0.3rem;">
            <span>${mon.sre_reviewed ? '✓' : '⏳'}</span> SRE Review ${mon.sre_reviewed ? 'Completed' : 'Pending'}
          </div>
          <span style="color:var(--text-muted);">➔</span>
          <div style="font-size:0.78rem; font-weight:600; color:${mon.sre_reviewed ? '#10b981' : 'var(--text-muted)'}; display:flex; align-items:center; gap:0.3rem;">
            <span>${mon.sre_reviewed ? '✓' : '○'}</span> SRE Approved
          </div>
          <span style="color:var(--text-muted);">➔</span>
          <div style="font-size:0.78rem; font-weight:600; color:${mon.sre_reviewed ? '#818cf8' : 'var(--text-muted)'}; display:flex; align-items:center; gap:0.3rem;">
            <span>○</span> Go-Live Readiness Validation
          </div>
        </div>

        ${!mon.sre_reviewed ? `
          <div style="border-top:1px solid var(--border-color); padding-top: 1rem; display: flex; justify-content: flex-end; gap: 0.5rem; align-items: center;">
            <label for="mon-sre-reviewer" style="font-size:0.8rem; color:var(--text-secondary);">SRE Reviewer Email: </label>
            <input type="email" id="mon-sre-reviewer" value="sre-oncall@company.com" style="width: 220px; background:var(--bg-tertiary); border: 1px solid var(--border-color); color:var(--text-primary); padding: 0.35rem 0.5rem; border-radius: 4px; font-size:0.8rem;">
            <button class="ops-btn" id="ops-sre-approve-btn">Approve Monitoring Plan (${mon.monitoring_plan_id || 'MON-PLAN-' + suffix})</button>
          </div>
        ` : `
          <div style="border-top:1px solid var(--border-color); padding-top: 1rem; font-size: 0.8rem; color: var(--color-status-green-text); font-weight: bold; text-align: right;">
    ...
    resultsHtml = section1_Scope + section2_Strategy + section3_Coverage + section4_Alerts + section5_Dashboard + section6_Summary + section7_Workflow;
  }
...
async function generateMonitoringConfig() {
  ...
  btn.disabled = true;
  btn.textContent = 'Generating AI Monitoring Plan...';
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = 'Generate AI Monitoring Plan';
  }
}
...
function renderHandoverKT() {
  ...
  if (opsPlan && opsPlan.tasks) {
    ...
    opsPlan.tasks.forEach(t => {
      const ownerVal = t.owner || t.owners || t.assigned_to;
      if (ownerVal) {
        const rawList = Array.isArray(ownerVal) ? ownerVal : String(ownerVal).split(',');
        rawList.forEach(o => {
          let email = String(o).trim().replace(/^\.+|\.+$/g, '');
          if (email) {
            if (!email.includes('@')) {
              email = email.toLowerCase().replace(/\s+/g, '.') + '@example.com';
            }
            ownersSet.add(email);
          }
        });
      ...
      <div style="padding: 1.25rem; overflow-y: auto; flex: 1; font-size: 0.82rem; line-height: 1.5; color: var(--text-primary);">
        ...
        <div style="background: rgba(0,0,0,0.15); border: 1px solid var(--border-color); padding: 0.85rem; border-radius: 6px; margin-bottom: 1.25rem;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
            ...
            <div>
              <div><strong>Delivery Team Contacts:</strong> <code>${(ho.delivery_team && ho.delivery_team.length > 0 ? ho.delivery_team.join(', ') : 'bob@example.com, karthik@example.com, fardeen@example.com, alice@example.com, john@example.com, raj@example.com, diana@example.com')}</code></div>
