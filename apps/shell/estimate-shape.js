...
function renderEstimateWizard(est) {
  ...
  const isApproved = est.status === 'approved' || est.status === 're-baselined' || est.status === 'finalized';
          ...
          <div class="wizard-step-body">
            ${isRebaselined ? `
              ...
              <div class="data-item" style="margin-top: 1rem; grid-column: span 2;">
                <div class="data-label">Re-baseline Reason</div>
...
async function runTriggerFlow(id) {
  ...
  try {
    ...
    if (data.rebaseline_warranted) {
        ...
        `;
        document.getElementById('btn-final-approve').addEventListener('click', () => finalApproveEstimate(id));
...
async function finalApproveEstimate(id) {
  const actionRow = document.getElementById('trigger-actions-row');
  if (actionRow) {
    actionRow.innerHTML = `<span class="loader"><span class="spinner"></span> Finalizing...</span>`;
  }
  try {
    const res = await fetch(`${ESTIMATE_API_BASE}/estimates/${id}/finalize`, { method: 'POST' });
